...
export const listSequences = defineTool({
  ...
  args: z.object({
    status: z.enum(["draft", "active", "paused", "archived"]).optional().describe("Filter by status. If omitted, returns all non-archived sequences."),
  ...
  handler: async (db, args) => {
    ...
    for (const seq of result) {
      ...
      text += `| \`${seq.id}\` | ${seq.name} | ${seq.status} | ${emailCount.length} | ${prospectCount.length} |\n`;
    }
