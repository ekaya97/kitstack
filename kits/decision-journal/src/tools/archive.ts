...
export const archive = withToolMetadata(defineTool({
  ...
  handler: async (ctx, args) => {
    ...
    switch (args.type) {
      case "decision": {
        const existing = await ctx.db.select({ id: decisions.id }).from(decisions).where(eq(decisions.id, args.id)).limit(1);
        if (existing.length === 0) return kit.error(`Decision "${args.id}" not found.`);
        await ctx.db.update(decisions).set({ archivedAt: now, updatedAt: now }).where(eq(decisions.id, args.id));
        break;
      }
      case "principle": {
        const existing = await ctx.db.select({ id: principles.id }).from(principles).where(eq(principles.id, args.id)).limit(1);
        if (existing.length === 0) return kit.error(`Principle "${args.id}" not found.`);
        await ctx.db.update(principles).set({ archivedAt: now, updatedAt: now }).where(eq(principles.id, args.id));
        break;
      }
    }
