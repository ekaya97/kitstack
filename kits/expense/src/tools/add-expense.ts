...
export const addExpense = defineTool({
  name: "add_expense",
  description: "Add a single expense with auto-calculated VAT fields (default 19%)",
  args: z.object({
    ...
    vatRate: z.number().optional().default(0.19).describe("VAT rate (0, 0.07, or 0.19)"),
  ...
  handler: async (db, args, ctx) => {
    ...
    return kit.text(
      `Expense added: "${args.description}" — €${args.amountGross.toFixed(2)} gross (€${Math.round(amountNet * 100) / 100} net, ${(args.vatRate * 100).toFixed(0)}% VAT)${receiptNote}. ID: \`${id}\`\n\nReminder: This is not tax advice. Verify with your Steuerberater.`
    );
  },
});
