...
export const categorize = defineTool({
  name: "categorize",
  description: "Auto-categorize uncategorized expenses by matching descriptions to German business categories and SKR03 accounts",
  args: z.object({
  ...
  handler: async (db, args, ctx) => {
    ...
    if (rows.length === 0) {
      return kit.text(args.expenseId ? "Expense not found or already categorized." : "No uncategorized expenses found.");
    }
