...
async function loadExpenses(db: LibSQLDatabase, args: z.infer<typeof listExpensesArgs>, ctx: KitContext) {
  const conditions = [];
...
export const listExpenses = defineTool({
  ...
  handler: async (db, args, ctx) => {
    const result = await loadExpenses(db, args, ctx);
    if (result.length === 0) {
      return kit.text("No expenses found matching the filters.");
    }
