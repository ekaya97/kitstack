...
export const quarterlySummary = defineTool({
  ...
  description: "Generate and store a quarterly summary of expenses with category breakdown and flagged items",
  args: z.object({
  ...
  handler: async (db, args, ctx) => {
    ...
    if (rows.length === 0) {
      return kit.text(`No expenses found for Q${args.quarter} ${args.year}.`);
    }
