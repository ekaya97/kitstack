...
export const updateExpense = defineTool({
  ...
  handler: async (db, args, ctx) => {
    const existing = await db.select().from(expenses).where(eq(expenses.id, args.expenseId)).then((r) => r[0]);
    if (!existing) {
      return kit.notFound("expense", args.expenseId);
    }
    ...
    return kit.text(`Expense "${existing.description}" updated — ${changes}.`);
  },
});
