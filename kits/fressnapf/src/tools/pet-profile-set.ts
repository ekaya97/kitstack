...
const petProfileSetArgs = z.object({
...
async function loadPetProfileSet(ctx: KitContext, args: z.infer<typeof petProfileSetArgs>) {
...
export const petProfileSet = defineTool({
  ...
  args: petProfileSetArgs,
  load: loadPetProfileSet,
  handler: async (ctx, args) => {
    const { id } = await loadPetProfileSet(ctx, args);
