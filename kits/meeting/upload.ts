/**
 * Upload Meeting kit build artifacts to S3 KitAssets bucket.
 * Run from project root: npx sst shell -- npx tsx kits/meeting/upload.ts
 */
...
async function upload() {
  ...
  console.log("Uploading Meeting kit...\n");

  // Upload views directory if it exists
  if (existsSync(viewsDir)) {
    count += await uploadDir(bucketName, viewsDir, "apps/kits/meeting-action-tracker");
  }

  // Upload generated shell
  const shellPath = resolve(buildDir, "shell.html");
  if (existsSync(shellPath)) {
    await uploadFile(bucketName, shellPath, "apps/kits/meeting-action-tracker/shell.html");
    count++;
  }
  ...
  if (existsSync(serverPath)) {
    await uploadFile(bucketName, serverPath, "apps/kits/meeting-action-tracker/kit.mjs");
    count++;
  }
