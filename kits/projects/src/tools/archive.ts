  ...
  handler: async (ctx, args) => {
    ...
    if (args.entity_type === "client") {
      const [row] = q.startsWith("cli_")
        ? await ctx.db.select().from(clients).where(eq(clients.id, q))
        : await ctx.db.select().from(clients).where(like(clients.name, `%${q}%`)).limit(1);
      if (!row) return kit.notFound("client", q);
      await ctx.db.update(clients).set({ archivedAt: now, updatedAt: now }).where(eq(clients.id, row.id));
    ...
    if (args.entity_type === "project") {
      const [row] = q.startsWith("prj_")
        ? await ctx.db.select().from(projects).where(eq(projects.id, q))
        : await ctx.db.select().from(projects).where(like(projects.name, `%${q}%`)).limit(1);
      if (!row) return kit.notFound("project", q);
      await ctx.db.update(projects).set({ archivedAt: now, updatedAt: now }).where(eq(projects.id, row.id));
    ...
    if (args.entity_type === "task") {
      const [row] = q.startsWith("tsk_")
        ? await ctx.db.select().from(tasks).where(eq(tasks.id, q))
        : await ctx.db.select().from(tasks).where(like(tasks.title, `%${q}%`)).limit(1);
      if (!row) return kit.notFound("task", q);
      // Tasks don't have archivedAt — mark as done with a note
      await ctx.db.update(tasks).set({ status: "done", completedDate: now.split("T")[0], updatedAt: now })
