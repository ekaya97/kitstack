...
export class DebriefService {
  ...
  private async transition(id: string, state: DebriefState): Promise<DebriefSession> { const s = this.require(id); const allowed = s.state === "prepared" && state === "calling" || s.state === "calling" && state === "awaiting_confirmation" || s.state === "awaiting_confirmation" && state === "partial" || (s.state === "awaiting_confirmation" || s.state === "partial") && state === "confirmed"; if (!allowed) throw new Error(`Invalid debrief transition ${s.state} -> ${state}`); s.state = state; s.updatedAt = this.now(); const operation = state === "confirmed" ? "complete" : state === "awaiting_confirmation" ? "await_confirmation" : "state"; await this.emit(s, operation, state === "partial" ? "partial" : "success"); return this.getSession(id); }
