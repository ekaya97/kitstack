...
export interface VoiceSimulatorOptions {
  debrief: { markCalling(sessionId: string): Promise<unknown>; awaitConfirmation(sessionId: string): Promise<unknown>; confirmDebrief(sessionId: string): Promise<unknown>; markFailed(sessionId: string, error: unknown): Promise<unknown>; getSession(sessionId: string): { state: DebriefSession["state"] } };
...
export class VoiceSimulator {
  ...
  async start(sessionId: string): Promise<VoiceResult> { await this.options.debrief.markCalling(sessionId); this.calls.set(sessionId, "calling"); await this.emit(sessionId, "start", "started"); return this.result(sessionId, "calling", turns(sessionId)); }
  ...
  async complete(sessionId: string, completion: Completion): Promise<VoiceStatusResult> { this.requireCall(sessionId); if (completion === "confirmed") { await this.options.debrief.confirmDebrief(sessionId); this.calls.set(sessionId, "confirmed"); await this.emit(sessionId, "complete", "success"); } else { this.calls.set(sessionId, "partial"); await this.emit(sessionId, "partial", "partial"); } return this.status(sessionId); }
